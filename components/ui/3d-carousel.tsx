      ...
      >
        ...
        >
          {cards.map((imgUrl, i) => (
            <motion.div
              key={`key-${imgUrl}-${i}`}
              className="absolute flex h-full origin-center items-center justify-center rounded-xl bg-white/10 backdrop-blur-sm p-4 shadow-lg border border-white/20"
              style={{
                width: `${faceWidth}px`,
                transform: `rotateY(${
                  i * (360 / faceCount)
            ...
            >
              <motion.img
                src={imgUrl}
                alt={`quartiere_${i} ${imgUrl}`}
                layoutId={`img-${imgUrl}`}
                className="pointer-events-none w-full rounded-xl object-cover aspect-square hover:scale-105 transition-transform duration-300"
...
function ThreeDPhotoCarousel({ 
  ...
  useEffect(() => {
    console.log("Quartieri caricati:", cards)
  }, [cards])
  ...
  const handleClick = (imgUrl: string, index: number) => {
    ...
    // Chiama la callback per aprire il dialog con le informazioni del quartiere
